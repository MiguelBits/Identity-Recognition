...
def train_faces():
    ...
    BASE_DIR = os.path.dirname(os.path.abspath("\src\images"))
...
def camera():  
    ...
    my_file = Path("\src\images\%s\myIMG%s.png"%(strMid_.lower(), filenumber))

    if my_file.is_file():
        copyfile("\myIMG.png", "\src\images\%s\myIMG%s.png"%(strMid_.lower(), randint(0,999)))
    else:
        copyfile("\myIMG.png", "\src\images\%s\myIMG%s.png"%(strMid_.lower(), filenumber))
